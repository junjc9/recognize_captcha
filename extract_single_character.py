...
# 获取我们需要处理的所有验证码图像的列表
img_input_captcha_dataset = glob.glob(os.path.join(IMG_DIR_INPUT_CAPTCHA, "*"))
counts = {}

# loop over the image paths
# 在图像路径上遍历
for (i, img_captcha) in enumerate(img_input_captcha_dataset):
    print("[INFO] 正在处理 {}/{}".format(i + 1, len(img_input_captcha_dataset)))

    # Since the filename contains the captcha text (i.e. "2A2X.png" has the text "2A2X"),
    # grab the base filename as the text
    # 由于文件名包含验证码文本（即“2A2X.png”具有文本“2A2X”），
    # 抓取基本文件名作为文本
    filename = os.path.basename(img_captcha)
    ...
    # 将每个字母保存为单个图像
    for img_output_character_bounding_box, letter_text in zip(img_output_character, captcha_correct_text):
        ...
        # 获取用于保存图像的文件夹
        save_path = os.path.join(IMG_DIR_OUTPUT_CHARACTER, letter_text)
